...
public class PayloadConverter implements com.radynamics.dallipay.browserwalletbridge.PayloadConverter {
    public JSONObject toJson(Transaction t) {
        ...
        if (ccy.getCode().equals(t.getLedger().getNativeCcySymbol())) {
            json.put("Amount", String.valueOf(t.getLedger().toSmallestUnit(t.getAmount())));
        } else {
        ...
        json.put("Fee", String.valueOf(t.getLedger().toSmallestUnit(FeeHelper.get(t.getFees(), FeeType.LedgerTransactionFee).orElseThrow())));
