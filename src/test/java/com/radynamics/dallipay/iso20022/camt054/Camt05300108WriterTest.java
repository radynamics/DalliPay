...
import com.radynamics.dallipay.iso20022.camt054.camt05300108.generated.Document;
...
public class Camt05300108WriterTest {
    ...
    public void testCreate2PaymentsLedgerCcy() throws Exception {
        testCreate2Payments("XRP", "camt054/camt.053.001.08_testCreate2Payments.xml");
    }

    @Test
    public void testCreate2PaymentsUsd() throws Exception {
        testCreate2Payments("USD", "camt054/camt.053.001.08_testCreate2Payments.USD.xml");
    }
    ...
    private void testCreate2Payments(String targetCcy, String expectationResourceName) throws Exception {
        ...
        var w = new Camt05300108Writer(cryptoInstruction.getLedger(), cryptoInstruction, ProductVersion);
    ...
    public void createRelatedPartiesNm() throws Exception {
        ...
        var w = new Camt05300108Writer(ti.getLedger(), ti, ProductVersion);
        w.setIdGenerator(new FixedValueIdGenerator());
        w.setCreationDate(TestFactory.createCreationDate());
    ...
    public void multipleCurrencies() throws Exception {
        ...
        {
            var ntfctn = actual.getBkToCstmrStmt().getStmt().get(0);
        ...
        {
            var ntfctn = actual.getBkToCstmrStmt().getStmt().get(1);
            Assertions.assertEquals(1, ntfctn.getNtry().size());
    ...
    public void RmtInfUstrdLength() throws Exception {
        ...
        {
            var ustrd = actual.getBkToCstmrStmt().getStmt().get(0).getNtry().get(0).getNtryDtls().get(0).getTxDtls().get(0).getRmtInf().getUstrd();
        ...
        {
            var ustrd = actual.getBkToCstmrStmt().getStmt().get(0).getNtry().get(1).getNtryDtls().get(0).getTxDtls().get(0).getRmtInf().getUstrd();
            Assertions.assertEquals(1, ustrd.size());
