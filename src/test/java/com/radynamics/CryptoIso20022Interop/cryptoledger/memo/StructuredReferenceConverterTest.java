...
public class StructuredReferenceConverterTest {
    ...
    public void toMemoQrr() {
        assertEquals("{\"t\":\"qrr\",\"v\":\"210000000003139471430009017\"}",
                StructuredReferenceConverter.toMemo(StructuredReferenceFactory.create("qrr", "210000000003139471430009017")).toString());
    }

    @Test
    public void toMemoScor() {
        assertEquals("{\"t\":\"scor\",\"v\":\"RF18539007547034\"}",
                StructuredReferenceConverter.toMemo(StructuredReferenceFactory.create("scor", "RF18539007547034")).toString());
        assertEquals("{\"t\":\"scor\",\"v\":\"RF18000000000539007547034\"}",
                StructuredReferenceConverter.toMemo(StructuredReferenceFactory.create("scor", "RF18000000000539007547034")).toString());
    }

    @Test
    public void toMemoIsr() {
        assertEquals("{\"t\":\"isr\",\"v\":\"814991000000006407610246697\"}",
                StructuredReferenceConverter.toMemo(StructuredReferenceFactory.create("isr", "814991000000006407610246697")).toString());
    }
}
